    ...
    .map(
      (citation) => `
        <li style="margin:0 0 10px 0; color:#c1c8d6; line-height:1.5;">
          <strong style="color:#ffffff;">${escapeHtml(stripMarkdown(citation.label))}</strong><br />
...
export function renderReportReviewText(report: Report) {
  const lines = [
    ...
    `${stripMarkdown(report.subtitle)}`,
    '',
    `Audience: ${stripMarkdown(report.audience)}`,
    `Edition: ${stripMarkdown(report.edition)}`,
    `Revision: ${stripMarkdown(report.revision)}`,
    ...
    '',
    'Implications:',
    ...report.implications.map((item) => `- ${stripMarkdown(item)}`),
    '',
    'Action steps:',
    ...report.actionSteps.map((item) => `- ${stripMarkdown(item)}`),
    '',
    'Risks / failure modes:',
    ...report.risks.map((item) => `- ${stripMarkdown(item)}`),
    '',
    'Review link:',
    ...
    .map(
      (item) => `
        <div style="padding:16px 0; border-top:1px solid #1f2937;">
          <div style="font-size:15px; font-weight:700; color:#ffffff; margin-bottom:6px;">${escapeHtml(stripMarkdown(item.icon))} ${escapeHtml(stripMarkdown(item.title))}</div>
          <div style="font-size:14px; line-height:1.6; color:#c1c8d6;">${escapeHtml(stripMarkdown(item.desc))}</div>
        </div>
      `,
    )
    .join('');
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:760px; margin:0 auto; background:#101827; border:1px solid #1f2937; border-radius:20px; overflow:hidden;">
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td style="padding-left:16px; vertical-align:middle;">
                ...
                <div style="font-size:26px; font-weight:800; color:#ffffff; margin-top:6px;">${escapeHtml(stripMarkdown(report.title))}</div>
                <div style="font-size:14px; color:#cbd5e1; margin-top:6px;">${escapeHtml(stripMarkdown(report.subtitle))}</div>
              </td>
            </tr>
          </table>
        </td>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td style="text-align:right; padding:0 0 10px 18px;">
                <div style="display:inline-block; padding:7px 12px; border-radius:999px; background:rgba(255,255,255,0.06); color:#e5e7eb; font-size:12px; font-weight:700;">${escapeHtml(stripMarkdown(report.revision))}</div>
              </td>
            </tr>
          </table>
        </td>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#0b1220; border:1px solid #1f2937; border-radius:16px;">
            <tr>
              <td style="padding:16px 18px; font-size:13px; color:#cbd5e1; line-height:1.7;">
                <strong style="color:#ffffff;">Edition:</strong> ${escapeHtml(stripMarkdown(report.edition))}<br />
                <strong style="color:#ffffff;">Audience:</strong> ${escapeHtml(stripMarkdown(report.audience))}<br />
      ...
      <tr>
        <td style="padding:18px 28px 8px 28px;">
          ...
          <div style="font-size:16px; line-height:1.7; color:#dbe3ee;">${escapeHtml(stripMarkdown(report.executiveSummary))}</div>
        </td>
      </tr>
      ...
      <tr>
        <td style="padding:18px 28px 8px 28px;">
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0">${renderList(report.implications, '#38bdf8')}</table>
        </td>
      </tr>
...
export function getReviewSendPayload(): ReportReviewPayload[] {
  return getAllReports().map((report) => {
    const html = renderReportReviewEmail(report);
    const qaIssues = runPremiumQualityChecks({
      ...
      freshnessTimestamp: report.freshnessTimestamp,
      renderedHtml: html,
    });

    return {
      slug: report.slug,
      subject: `${BRAND_NAME} review packet: ${stripMarkdown(report.title)} (${stripMarkdown(report.revision)})`,
      html,
