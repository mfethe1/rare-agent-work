...
import { joinSwarm } from '@/lib/a2a/swarm/engine';
import { joinSwarmSchema } from '@/lib/a2a/swarm/validation';

/**
 * POST /api/a2a/swarms/:swarmId/join — Join a swarm
 */
export async function POST(
  ...
  try {
    ...
    const result = await joinSwarm({ swarm_id: swarmId, agent_id: agent.id, input: parsed.data });
    if ('error' in result) {
    ...
    console.error('POST /api/a2a/swarms/:id/join error:', err);
