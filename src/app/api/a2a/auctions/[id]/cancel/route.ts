...
import { cancelAuction } from '@/lib/a2a/auctions';

/**
 * POST /api/a2a/auctions/:id/cancel — Cancel an open auction.
 *
 * Only the requester can cancel. Refunds full escrow and marks all bids as lost.
 * Cannot cancel after award.
 */
export async function POST(
  ...
  try {
    ...
    const result = await cancelAuction(id, agent.id);
    ...
    return NextResponse.json(safeErrorBody(err, 'db', 'POST /api/a2a/auctions/:id/cancel'), { status: 500 });
  }
}
